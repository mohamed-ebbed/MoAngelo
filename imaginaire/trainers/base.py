...
class BaseTrainer(object):
    ...
    def end_of_iteration(self, data, current_epoch, current_iteration):
        ...
        if current_iteration % self.cfg.logging_iter == 0:
            ...
            if "total" in self.losses:
                loss_str = f" | Loss: {self.losses['total'].item():.4f}"

            frame_str = ""
            if hasattr(self.cfg.data, "target_frame"):
                frame_str = f"[Frame {self.cfg.data.target_frame:03d}] "

            print(f"{frame_str}Iter: {current_iteration:06d}/{self.cfg.max_iter} | "
                  f"Time/Iter: {avg_time:.3f}s{loss_str}")
    ...
    def end_of_epoch(self, data, current_epoch, current_iteration):
        ...
        if hasattr(self.cfg.data, "target_frame"):
            frame_str = f"[Frame {self.cfg.data.target_frame:03d}] "
        print(f"{frame_str}Epoch: {current_epoch:03d} | Total Time: {elapsed_epoch_time:.2f}s")
