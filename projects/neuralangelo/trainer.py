...
class Trainer(BaseTrainer):
    ...
    def __init__(self, cfg, is_inference=True, seed=0):
        ...
        if not self.cfg.data.canonical:
            canonical_state_dict = torch.load(self.cfg.model.object.canonical_dir)["model"]
            self.model.load_state_dict(canonical_state_dict, strict=False)
        if cfg.model.object.sdf.encoding.type == "hashgrid" and cfg.model.object.sdf.encoding.coarse2fine.enabled:
            ...
            if cfg.data.canonical:
                ...
                self.model.module.neural_deform.warm_up_end = self.warm_up_end
                self.c2f_step = cfg.model.object.deform.encoding.coarse2fine.step
                print(f"c2f_step: {self.c2f_step}")


    def _init_loss(self, cfg):
        self.criteria["render"] = torch.nn.L1Loss()
        self.criteria["mask"] = torch.nn.BCELoss()
    ...
    def _compute_loss(self, data, mode=None):
        if mode == "train":
            ...
            if not self.cfg.model.background.enabled:
                self.losses["mask"] = self.criteria["mask"](data["weights_sum"].squeeze(-1).clip(1e-3, 1.0 - 1e-3), data["mask_sampled"].float())
    ...
    def _start_of_iteration(self, data, current_iteration):
        ...
        if self.cfg.model.object.sdf.encoding.coarse2fine.enabled:
            ...
            if not self.cfg.data.canonical:
                model.neural_deform.set_active_levels(current_iteration)
            if self.cfg_gradient.mode == "numerical":
                model.neural_sdf.set_normal_epsilon(canonical=self.cfg.data.canonical)
    ...
    def log_wandb_scalars(self, data, mode=None):
        ...
        if self.cfg.model.object.sdf.encoding.coarse2fine.enabled:
            scalars[f"{mode}/active_levels_sdf"] = self.model.module.neural_sdf.active_levels
            if not self.cfg.data.canonical:
                scalars[f"{mode}/active_levels_deform"] = self.model.module.neural_deform.active_levels
        step = getattr(self, "global_step_offset", 0) + self.current_iteration
