...
from projects.neuralangelo.utils.modules import NeuralSDF, NeuralRGB, BackgroundNeRF, NeuralDeform
from projects.neuralangelo.utils.pytorch3d import transform_points
...
class Model(BaseModel):
    ...
    def __init__(self, cfg_model, cfg_data):
        ...
        self.canonical = cfg_data.canonical
        try:
            self.tune_template = cfg_model.object.deform.tune_template
        except:
            self.tune_template = False
        # Define models.
    ...
    def build_model(self, cfg_model, cfg_data):
        ...
        # Define the neural deformation model.  
        if not self.canonical:                 
            self.neural_deform = NeuralDeform(cfg_model.object.deform)
        else:
            self.neural_deform = lambda x, t: torch.zeros((x.shape[0],x.shape[1],x.shape[2],6)).to(x.device)  # [B,R,N,3]
        if cfg_model.background.enabled:
    ...
    def forward(self, data):
        ...
        output = self.render_pixels(data["pose"], data["intr"], data["time_step"], image_size=self.image_size_train,
    ...
    def inference(self, data):
        ...
        output = self.render_image(data["pose"], data["intr"], data["time_step"], image_size=self.image_size_val,
    ...
    def render_image(self, pose, intr, time_step, image_size, stratified=False, sample_idx=None):
        ...
        for center, ray, _ in self.ray_generator(pose, intr, image_size, full_image=True):
            ...
            output_batch = self.render_rays(center, time_step, ray_unit, sample_idx=sample_idx, stratified=stratified)
    ...
    def render_pixels(self, pose, intr, time_step, image_size, stratified=False, sample_idx=None, ray_idx=None):
        ...
        ray_unit = torch_F.normalize(ray, dim=-1)  # [B,R,3]
        output = self.render_rays(center, time_step, ray_unit, sample_idx=sample_idx, stratified=stratified)
        return output

    def render_rays(self, center, time_step, ray_unit, sample_idx=None, stratified=False):
        with torch.no_grad():
        ...
        output_object = self.render_rays_object(center, time_step, ray_unit, near, far, outside, app, stratified=stratified)
        if self.with_background:
            output_background = self.render_rays_background(center, time_step, ray_unit, far, app_outside, stratified=stratified)
        ...
        output = dict(
            ...
            weights_sum=weights.sum(dim=2, keepdim=True),  # [B,R,1]
    ...
    def get_param_groups(self, cfg_optim):
        base_lr = getattr(cfg_optim, "base_lr", getattr(cfg_optim.params, "lr", 1e-3))

        if self.canonical:
            return [{'params': self.parameters(), 'name': 'all', 'lr': base_lr}]
        else:
            if self.tune_template:
                return [{'params': self.parameters(), 'name': 'all', 'lr': base_lr}]
            else:
                return [{'params': self.neural_deform.parameters(), 'name': 'deform', 'lr': base_lr}]

    def render_rays_object(self, center, time_step, ray_unit, near, far, outside, app, stratified=False):
        with torch.no_grad():
            dists = self.sample_dists_all(center, ray_unit, near, far, stratified=stratified)  # [B,R,N,3]
        points_o = camera.get_3D_points_from_dist(center, ray_unit, dists)  # [B,R,N,3]
        deform = self.neural_deform(points_o, time_step)
        points, transform = transform_points(points_o, deform)  # [B,R,N,3]
        ...
        gradients, gradients_c, hessians = self.neural_sdf.compute_gradients(points_o, time_step, self.neural_deform, training=self.training, sdf=sdfs)
        normals = torch_F.normalize(gradients, dim=-1)  # [B,R,N,3]
        normals_c = torch_F.normalize(gradients_c, dim=-1)
        translation = transform[..., :3, 3]
        rgbs = self.neural_rgb.forward(points, normals_c, rays_unit, feats, app=app)  # [B,R,N,3]
        # SDF volume rendering.
        alphas = self.compute_neus_alphas(ray_unit, sdfs, gradients_c, dists, dist_far=far[..., None],
    ...
    def render_rays_background(self, center, time_step, ray_unit, far, app_outside, stratified=False):
        with torch.no_grad():
        ...
        deform = self.neural_deform(points, time_step)
        points, transform = transform_points(points, deform)
    ...
    def sample_dists_all(self, center, ray_unit, near, far, stratified=False):
        ...
        if self.cfg_render.num_sample_hierarchy > 0:
            points = camera.get_3D_points_from_dist(center, ray_unit, dists)  # [B,R,N,3]
            deform = self.neural_deform(points, -1)
            points, transform = transform_points(points, deform)  # [B,R,N,3]
        ...
        for h in range(self.cfg_render.num_sample_hierarchy):
            ...
            if h != self.cfg_render.num_sample_hierarchy - 1:
                ...
                deform = self.neural_deform(points_fine, -1)
                points_fine, transform = transform_points(points_fine, deform)  # [B,R,N,3]
