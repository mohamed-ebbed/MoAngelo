...
@torch.no_grad()
def extract_mesh(sdf_func, deform_field, bounds, intv, time_step, block_res=64, texture_func=None, filter_lcc=False):
    ...
    for it, data in enumerate(data_loader):
        xyz = data["xyz"][0]
        xyz_cuda = xyz.cuda()
        time_emb_cuda = torch.tensor([time_step], device=xyz_cuda.device).long().expand(xyz_cuda.shape[0])
        xyz_deformed, _ = transform_points(xyz_cuda, deform_field(xyz_cuda, time_emb_cuda))
        #xyz_deformed = xyz_cuda
        sdf_cuda = sdf_func(xyz_deformed)[..., 0]
        sdf = sdf_cuda.cpu()
...
def extract_texture(xyz, neural_rgb, neural_sdf, appear_embed):
    ...
    gradients, _ = neural_sdf.compute_gradients(xyz_cuda, time_step, deform_field, training=False, sdf=sdfs)
