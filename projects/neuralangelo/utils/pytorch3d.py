"""
Taken from nersemble, thanks tobias <3 <3
...
def transform_points(points, log_transform):
    if log_transform.shape[-1] == 6:
        #points has shape of (B, R, N, 3)
        log_transform = log_transform.reshape(-1, 6)  # Flatten the batch and ray dimensions
        transform = se3_exp_map(log_transform)
        B, R, N, _ = points.shape
        points = points.reshape(B * R * N, 3)  # Flatten the batch and ray dimensions
        transform = transform.reshape(B * R * N, 4, 4)  # Flatten the batch and ray dimensions
        # Transform points using the SE(3) matrix
        points_h = torch.cat([points, torch.ones(points.shape[0], 1, dtype=points.dtype, device=points.device)], dim=-1).unsqueeze(-1)  # Convert to homogeneous coordinates
        transformed_points = torch.bmm(transform, points_h).squeeze()[:, :-1]  # Apply the transformation
        transformed_points = transformed_points.reshape(B, R, N, 3)  # Reshape back to original dimensions
    else:
        transformed_points = points + log_transform
    return transformed_points, transform.reshape(B, R, N, 4, 4)  # Reshape back to original dimensions

def transform_normals(normals, transform):
    B, R, N, _ = normals.shape
    normals = normals.reshape(B * R * N, 3, 1)  # Flatten the batch and ray dimensions
    transform = transform.reshape(B * R * N, 4, 4)  # Flatten the batch and ray dimensions
    rot = transform[:, :3, :3]  # Extract the rotation part of the SE(3) matrix
    transformed_normals = torch.bmm(rot, normals).squeeze(-1)  # Apply the transformation
    transformed_normals = transformed_normals.reshape(B, R, N, 3)  # Reshape back to original dimensions
