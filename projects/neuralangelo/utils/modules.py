...
from projects.neuralangelo.utils.pytorch3d import transform_points, transform_normals
...
class NeuralSDF(torch.nn.Module):
    ...
    def set_normal_epsilon(self, canonical=True):
        if self.cfg_sdf.encoding.coarse2fine.enabled and canonical:
    ...
    def compute_gradients(self, x, time_step, deformation_field, training=False, sdf=None):
        ...
        if self.cfg_sdf.gradient.mode == "analytical":
            ...
            with torch.enable_grad():
                ...
                deform = deformation_field(x, time_step)
                x_c, transform = transform_points(x, deform)
                sdf = self.sdf(x_c)
                gradient = torch.autograd.grad(sdf.sum(), x, create_graph=True)[0]
                gradient_c = transform_normals(gradient, transform)  # [...,3]
            ...
            if self.cfg_sdf.gradient.taps == 6:
                ...
                sdf_x_pos = self.sdf(x + eps_x + deformation_field(x + eps_x, time_step))  # [...,1]
                sdf_x_neg = self.sdf(x - eps_x + deformation_field(x - eps_x, time_step))  # [...,1]
                sdf_y_pos = self.sdf(x + eps_y + deformation_field(x + eps_y, time_step))  # [...,1]
                sdf_y_neg = self.sdf(x - eps_y + deformation_field(x - eps_y, time_step))  # [...,1]
                sdf_z_pos = self.sdf(x + eps_z + deformation_field(x + eps_z, time_step))  # [...,1]
                sdf_z_neg = self.sdf(x - eps_z + deformation_field(x - eps_z, time_step))  # [...,1]
                ...
                sdf1 = self.sdf(x + k1 * eps + deformation_field(x + k1 * eps, time_step))  # [...,1]
                sdf2 = self.sdf(x + k2 * eps + deformation_field(x + k2 * eps, time_step))  # [...,1]
                sdf3 = self.sdf(x + k3 * eps + deformation_field(x + k3 * eps, time_step))  # [...,1]
                sdf4 = self.sdf(x + k4 * eps + deformation_field(x + k4 * eps, time_step))  # [...,1]
        ...
        return gradient, gradient_c, hessian
...
class NeuralDeform(torch.nn.Module):


    def __init__(self, cfg_deform):
        super().__init__()
        self.cfg_deform = cfg_deform
        self.encoding_dim = self.build_encoding(cfg_deform.encoding)
        input_dim = 3 + self.encoding_dim
        self.build_mlp(cfg_deform.mlp, input_dim=input_dim)
    ...
    def encode(self, points_3D, frame_idx=-1):
        if self.cfg_deform.encoding.type == "fourier":
            points_enc = nerf_util.positional_encoding(points_3D, num_freq_bases=self.cfg_deform.encoding.levels)
            feat_dim = 6
        elif self.cfg_deform.encoding.type == "hashgrid":
            # Tri-linear interpolate the corresponding embeddings from the dictionary.
            vol_min, vol_max = self.cfg_deform.encoding.hashgrid.range
            points_3D_normalized = (points_3D - vol_min) / (vol_max - vol_min)  # Normalize to [0,1].
            tcnn_input = points_3D_normalized.view(-1, 3)
            tcnn_output = self.tcnn_encoding(tcnn_input)
            points_enc = tcnn_output.view(*points_3D_normalized.shape[:-1], tcnn_output.shape[-1])
            feat_dim = self.cfg_deform.encoding.hashgrid.dim
        # Coarse-to-fine.
        if self.cfg_deform.encoding.coarse2fine.enabled:
            mask = self._get_coarse2fine_mask(points_enc, feat_dim=feat_dim)
            points_enc = points_enc * mask
        points_enc = torch.cat([points_3D, points_enc], dim=-1)  # [B,R,N,3+LD]
        return points_enc
    ...
    def set_active_levels(self, current_iter=None):
        anneal_levels = max((current_iter - self.warm_up_end) // self.cfg_deform.encoding.coarse2fine.step, 1)
        self.anneal_levels = min(self.cfg_deform.encoding.levels, anneal_levels)
        self.active_levels = int(max(self.cfg_deform.encoding.coarse2fine.init_active_level, self.anneal_levels))

    def build_mlp(self, cfg_mlp, input_dim=3):
        # RGB prediction
        layer_dims = [input_dim] + [cfg_mlp.hidden_dim] * cfg_mlp.num_layers + [cfg_mlp.output_dim]
        activ = get_activation(cfg_mlp.activ, **cfg_mlp.activ_params)
        self.mlp = nerf_util.MLPwithSkipConnection(layer_dims, skip_connection=cfg_mlp.skip, activ=activ,
                                                   use_weightnorm=cfg_mlp.weight_norm)
        nn.init.zeros_(self.mlp.linears[-1].weight)
        nn.init.zeros_(self.mlp.linears[-1].bias)

    def forward(self, points_3D, time_step):
        pos_enc = self.encode(points_3D, time_step)  # [...,LD]
        input_list = [pos_enc]
        input_vec = torch.cat(input_list, dim=-1)
        deform = self.mlp(input_vec)
        return deform  # [...,3]

    def encode_pos(self, rays_unit):
        if self.cfg_deform.encoding_position.type == "fourier":
            pos_enc = nerf_util.positional_encoding(rays_unit, num_freq_bases=self.cfg_deform.encoding_position.levels)
        elif self.cfg_rgb.encoding_view.type == "spherical":
            pos_enc = self.spherical_harmonic_encoding(rays_unit)
        else:
            raise NotImplementedError("Unknown encoding type")
        return pos_enc

    # def set_active_levels(self, curr_iter, max_iter=100000):
    #     alpha = 4 * curr_iter / max_iter
    #     freq = torch.arange(0,4).cuda()
    #     freq = torch.repeat_interleave(freq,6).cuda(0).reshape((1,1,1,-1))
    #     self.weights = (1 - torch.cos(np.pi * torch.clamp(alpha - freq, 0, 1))) / 2
