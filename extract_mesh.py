...
def main():
    ...
    if cfg.model.object.sdf.encoding.coarse2fine.enabled:
        ...
        if not cfg.data.canonical:
            trainer.model.module.neural_deform.set_active_levels(trainer.current_iteration)
    ...
    if not cfg.data.canonical:                 
        deform_field = trainer.model_module.neural_deform
    else:
        deform_field = lambda x, t: torch.zeros_like(x).to(x.device)
    ...
    mesh = extract_mesh(sdf_func=sdf_func, deform_field=deform_field, time_step = -1, bounds=bounds, intv=(2.0 / args.resolution),
    ...
    if is_master():
        ...
        mesh.export(os.path.join(args.output_file))
